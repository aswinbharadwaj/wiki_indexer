...
public class ThreadedIndexerRunner {
	...
	private class RunnerThread implements Runnable {
		...
		private RunnerThread(int pnum) {
			...
			writer = new IndexWriter(props, INDEXFIELD.TERM, INDEXFIELD.LINK);   // the type of indexer <term id,doc id>
		...
		public void run() {
			...
			while (true) {
				...
				if (etr == null) {
					if (isComplete) {
						try {
							System.out.println("All the Term index is written, now calling the final write to disk");
							writer.writeToDisk();    // once the whole collection of documents is written the write to disk is called to store the index
						} catch (IndexerException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
						System.out.println("All the Term index is written, now calling the cleanup to read and merge and re write into the disk");
						writer.cleanUp();
					...
					// we have an entry
					try {
						//System.out.println(etr.term+" "+etr.docId+" "+etr.numOccurances);
						writer.addToIndex(etr.term, etr.docId, // we call the add to index of the index writer to add the document id to the term's postings list with the number of occurrences in that document
								etr.numOccurances);
					} catch (IndexerException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
