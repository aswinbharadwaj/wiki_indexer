...
public class Runner {
	...
	private static void runIndexer(Properties properties) {
		...
		/*
		 * Use the code below and comment out the other code (except first line) for just running the parser alone (SAX+ Wiki). Displays the time in nano
		 * seconds. ~8 seconds for 1600 docs .. it is too high, needs optimizations
		*/
		/*
		Parser p = new Parser(properties);
		long start = System.nanoTime();
		p.parse(FileUtil.getDumpFileName(properties), queue);
		System.out.println(System.nanoTime()-start);
		System.out.println(queue.size());
		int i = 0;
		for (WikipediaDocument wdoc: queue) {
			System.out.println(wdoc.getLinks());
		}
	...
	private static void tokenizeAndIndex(Properties properties,
		...
		try {
			while (remaining > 0) {
				idoc = pool.take().get(); // This is where the indexable document is returned by calling the call method of the document transformer ! 
				if (idoc != null) {
					currDocId = docDict.lookup(idoc.getDocumentIdentifier()); // Now the document dictionary will store the values of the <document id, id uniquely we generate> map as the dictionary
					TokenStream stream;
					try {
						for (INDEXFIELD fld : INDEXFIELD.values()) {
							/*
							 * for every field there is a stream of tokens taken from the wiki doc and tokenizer rules applied on them for the particular document:
							 * Term -> title, section title and text
							 * Author -> author
							 * Category -> category
							 * Link -> link
							 */
							...
							if (stream != null) {
								tokenmap = stream.getTokenMap(); // gets the token map <term,count> for the particular field

								if (tokenmap != null) {
									switch (fld) {
									case TERM:
										termRunner.addToIndex(tokenmap,
												currDocId);             // Calls the threaded runner to insert the terms(??) into the index. Note that the token
																		// map and the document id to actually add to the postings list of the term are sent 
																		// as parameters to the ass to index method
										break;
									case AUTHOR:
										authIdxer.processTokenMap(
												currDocId, tokenmap); // Calls the single runner to insert authors(?) into the author index
																	  // It takes up the current doc id, the author token map
										break;
									case CATEGORY:
										catIdxer.processTokenMap(currDocId,
												tokenmap);					// Calls the single runner to insert categories(?) into the category index
										  									// It takes up the current doc id and the category token map
										break;
									case LINK:
										linkIdxer.processTokenMap(
												currDocId, tokenmap); // Calls the single runner to insert link (?) into the link index
	  																  // It takes up the current doc id and the links token map
										break;
									}
								}
							}
